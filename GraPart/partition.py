...
def multiway_partitioning(xy, group_matrix, labels_list, connect_matrix):
    ...
    cluster_pairs = [(i,j) for i in range(num_clusters) for j in range(i+1,num_clusters) if group_matrix[i,j] == 1]
    for i,j in cluster_pairs:
        i,j = int(i), int(j)
        group_matrix, labels_list, INTERCHANGE = two_way_partitioning(i,j,xy,group_matrix,labels_list,connect_matrix)
        if INTERCHANGE:
    ...
    return group_matrix, labels_list

def one_way_partitioing(xy, group_matrix, labels_list, connect_matrix, margin=0.1):
    num_clusters = group_matrix.shape[0]
    pairs = [(i,j) for i in range(num_clusters) for j in range(num_clusters) if group_matrix[i,j] == 1]
    while pairs:
        i,j= pairs.pop()
        # d
        group_matrix, labels_list, INTERCHANGE = one_way_partitioing_single_dvalue(i,j,xy,group_matrix,labels_list,connect_matrix, original_k=num_clusters, margin=margin)
        if INTERCHANGE:
            # group j ate all the nodes in group i
            if num_clusters > group_matrix.shape[0]:
                pairs = [(i,j) for i in range(group_matrix.shape[0]) for j in range(i+1,group_matrix.shape[0]) if group_matrix[i,j] == 1]
            else:
                for idx in range(len(group_matrix[i])):
...
def bisection(xy,
    ...
    if return_results:
        return buffer, group_matrix, labels_list
    else:
        return buffer
...
if __name__ == "__main__":
    ...
    bisection(xy, max_clusters = 30, connect_distance = 1, max_firewalls = 50, variation='self')